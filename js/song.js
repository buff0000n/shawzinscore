...
class Song {
    ...
    split(intervalLength) {
        ...
        for (var i = 0; i < this.notes.length; i++) {
            ...
            // if the note's tick is after our current interval, add sub-songs and increment the interval
            // until we reach the right range
            while (note.tick >= intervalEnd) {
            ...
            // offset the note so that the interval start is tick zero
            subSong.notes.push(note.offsetTick(-intervalStart));
        }
...
function drawNotes(context, x1, y1, x2, y3, y2, fontSize, drawSong, intervalLength, clear, fret0, fret1, fret2, fret3, fretSep) {
    ...
    for (var n = 0; n < drawSong.notes.length; n++) {
        ...
        var x3 = x1 + (((x2 - x1) * note.tick) / (intervalLength - 1));
